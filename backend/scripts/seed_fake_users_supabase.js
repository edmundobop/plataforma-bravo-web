...
(async () => {
  try {
    ...
    const list = await query(
      `SELECT email, matricula FROM usuarios WHERE matricula LIKE 'CBM-90%' ORDER BY matricula LIMIT 10`
    );

    console.log('Inserções realizadas:', count.rows[0].total);
    console.table(list.rows);
    process.exit(0);
  } catch (e) {
    console.error('Falha ao inserir usuários fictícios:', e.message);
    process.exit(1);
  }
})();
